...
pub(crate) fn remove_range(
    ...
    if let Some(region) = guest_memory.find_region(guest_address) {
        ...
        // and it will no longer receive remove events or subsequent page faults for that memory range,
        // making it impossible for the UFFD handler to track removed pages.
        if restored && !uffd {
